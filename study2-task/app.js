(function () {
  ...
  const state = {
    schedule: [],
    participantIndex: null,   // set by buildSchedule; drives the role rotation
    currentTrialIndex: -1,
    currentTrial: null,
    namePool: [],
    currentDogNames: [],
  ...
  function updateHeader() {
    const a = STUDY.trajectoryAllocation;
    const planned = 3 * (a.nmPerTrajectory + a.nwPerTrajectory) +
      STUDY.clearAllocation.CW + STUDY.clearAllocation.CL;
    const total = state.schedule.length || planned;
  ...
  /*
   * Participant index, which drives the counterbalancing rotation.
   *
   * Read from ?pidx=N if present. Otherwise derived by hashing the Prolific ID,
   * which spreads participants deterministically across the rotation without
   * needing a server to hand out sequential numbers. Falls back to a random
   * index for local testing.
   *
   * Any single participant is perfectly balanced regardless of index (they always
   * get 2 NM + 2 NW per trajectory); the index only decides WHICH clip plays which
   * role, so that role counts even out across the sample.
   */
  function participantIndex() {
    const params = new URLSearchParams(window.location.search);
    const explicit = parseInt(params.get("pidx"), 10);
    ...
    if (pid) {
      ...
      for (let i = 0; i < pid.length; i += 1) {
        h = (h * 31 + pid.charCodeAt(i)) >>> 0; // simple deterministic string hash
      }
      return h;
    }
  ...
  // Shorten a session for testing: ?reps=1 gives 1 NM + 1 NW per trajectory and
  // 1 CW + 1 CL. The real Prolific link carries no param and runs the full set.
  function repsScale() {
  ...
  function makeTrial(race, cond) {
    return {
      condition: cond.code,
      conditionLabel: cond.label,
      role: cond.role,
      trajectory: race.trajectory || null,   // null for clear races
      raceId: race.id,
      contestedLabel: race.contested === true,
      ...
      assignedTrap: trapForRole(race, cond.role),
  ...
  /*
   * Rotation, not random draw.
   *
   * Within each trajectory the clips sit in a fixed order. Participant i takes the
   * (nm + nw) clips starting at position i mod k, assigns the first nm as near
   * misses and the rest as narrow wins. The next participant starts one position
   * later, so over a full cycle every clip has served in both roles the same
   * number of times.
   *
   * Study 1 randomised this and ended up with race 20 at 27 NM vs 11 NW, and 14%
   * of participants seeing no catch-up near miss at all. Rotation makes both
   * impossible by construction.
   */
  function rotateAssign(clips, index, nNM, nNW) {
    const k = clips.length;
    if (k < nNM + nNW) {
      throw new Error(`Need >= ${nNM + nNW} clips but only ${k} available for ${clips[0] && clips[0].trajectory}`);
    }
    const picked = [];
    for (let j = 0; j < nNM + nNW; j += 1) picked.push(clips[(index + j) % k]);
    return {
      nm: picked.slice(0, nNM),
      nw: picked.slice(nNM)
    };
  }

  /*
   * Refuse to run for a real participant while any race in the pool still has an
   * unverified runner count. The card is built from nRunners/missing, so a wrong
   * value would offer a dog that never ran in that race — silently corrupting the
   * trial. Local testing (no Prolific ID) is allowed through with a loud warning.
   */
  function assertPoolVerified() {
    const bad = STUDY.races.filter((r) => r.unverified === true).map((r) => r.id);
    if (!bad.length) return;
    const live = typeof Pavlovia !== "undefined" && Pavlovia.prolificPID;
    const msg = `Races ${bad.join(", ")} have UNVERIFIED runner counts / vacant traps. ` +
      "Confirm them against the footage and remove `unverified: true` before launch.";
    if (live) throw new Error(msg);
    console.warn("[study2] " + msg + " Continuing because this is not a Prolific session.");
  }

  function buildSchedule() {
    assertPoolVerified();
    const index = participantIndex();
    const scale = repsScale();
    const alloc = STUDY.trajectoryAllocation;
    const nNM = scale || alloc.nmPerTrajectory;
    const nNW = scale || alloc.nwPerTrajectory;
    const trials = [];

    // close races, one balanced block per trajectory
    const trajectories = ["catch-up", "fall-back", "stable"];
    trajectories.forEach((traj, t) => {
      const clips = STUDY.races.filter((r) => r.kind === "close" && r.trajectory === traj);
      // offset each trajectory differently so participant i is not always taking
      // position i in all three lists at once
      const { nm, nw } = rotateAssign(clips, index + t * 7, nNM, nNW);
      nm.forEach((race) => trials.push(makeTrial(race, STUDY.conditions.NM)));
      nw.forEach((race) => trials.push(makeTrial(race, STUDY.conditions.NW)));
    });

    // clear races supply CW and CL, drawn without replacement
    const clearPool = shuffle(STUDY.races.filter((r) => r.kind === "clear"));
    const nCW = scale || STUDY.clearAllocation.CW;
    const nCL = scale || STUDY.clearAllocation.CL;
    if (clearPool.length < nCW + nCL) {
      throw new Error(`Need ${nCW + nCL} clear races but only ${clearPool.length} available`);
    }
    for (let i = 0; i < nCW; i += 1) trials.push(makeTrial(clearPool.shift(), STUDY.conditions.CW));
    for (let i = 0; i < nCL; i += 1) trials.push(makeTrial(clearPool.shift(), STUDY.conditions.CL));
  ...
  function handleRaceEnded() {
    ...
    state.results.push({
      ...
      participantIndex: state.participantIndex,   // drives the counterbalancing rotation
      condition: trial.condition,
      trajectory: trial.trajectory || "",         // catch-up / fall-back / stable; blank on clear races
      contestedLabel: trial.contestedLabel === true, // race 16 only; pre-registered sensitivity analysis
      raceId: trial.raceId,
